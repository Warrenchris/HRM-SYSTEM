...
export function useSubmitExpenseMutation() {
  ...
  return useMutation({
    mutationFn: async (data: CreateExpenseData) => {
      const insertPayload: any = { ...data }; // cast to any so TS won't require DB-generated fields

      // Client-side expense number generator (fallback when DB trigger is missing)
      ...
      // Try up to 5 times to insert with a client-generated expense_number to avoid unique collisions
      for (let attempt = 0; attempt < 5; attempt++) {
        insertPayload.expense_number = generateExpenseNumber();
        ...
        if (res.error) {
          ...
          // If it's a duplicate key on expense_number, retry with a new value; otherwise fail fast
          if (msg.includes('duplicate') || msg.includes('expense_number')) {
            continue;
          }
          throw res.error;
        }
  ...
  if (!expense) throw lastError || new Error('Failed to insert expense');
